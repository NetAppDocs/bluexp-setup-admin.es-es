...
+
Si es necesario, seleccione *Agregar un miembro* para especificar miembros adicionales de la organización que deben tener acceso a la carpeta o el proyecto y, a continuación, seleccione un rol. Un rol de acceso define los permisos que tienen los miembros para la carpeta o el proyecto.
...
Si el acceso de miembros se ha proporcionado en un nivel superior de la jerarquía (en el nivel de carpeta u organización), no puede eliminar ni cambiar su rol al ver la carpeta o el proyecto de nivel inferior. Debe cambiar a esa parte de la jerarquía y actualizar allí el rol del miembro. Como alternativa, puede link:task-iam-manage-roles.html#manage-permissions["Gestione roles desde la página Miembros"].
