...
[role="lead"]
BlueXP ofrece varios _modos de implementación_ que le permiten utilizar BlueXP de forma que se adapte a sus necesidades empresariales y de seguridad. _Modo estándar_ aprovecha el software BlueXP  como capa de servicio (SaaS) para proporcionar una funcionalidad completa, mientras que _modo restringido_ y _modo privado_ están disponibles para organizaciones que tienen restricciones de conectividad.
