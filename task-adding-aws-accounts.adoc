...
Después de agregar sus credenciales de AWS a BlueXP , puede configurar una suscripción de AWS Marketplace con esas credenciales. La suscripción le permite pagar por Cloud Volumes ONTAP a una tarifa por hora (PAYGO) o mediante un contrato anual, y utilizar otros servicios de BlueXP.

Hay dos escenarios en los que puede configurar una suscripción a AWS Marketplace después de haber agregado las credenciales a BlueXP :

* No configuraste una suscripción cuando agregaste inicialmente las credenciales a BlueXP .
