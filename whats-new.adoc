...
image:screenshot-edit-name.png["Una captura de pantalla que muestra la capacidad de editar su nombre en Configuración de usuario."]

No se puede editar su nombre si inicia sesión con una conexión federada o con su cuenta del sitio de soporte de NetApp.



=== Requisito del servicio de metadatos de la instancia de Amazon EC2

Debe tener en cuenta que se requiere IMDSv1 en las instancias EC2 antes de instalar manualmente el software Connector en AWS.

link:reference-limitations.html["Obtenga más información sobre el requisito para IMDSv1 en AWS"].
