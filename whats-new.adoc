...
La Cronología ahora incluye eventos de inicio de sesión de los usuarios. Esto permite ver cuándo iniciaron sesión en BlueXP, lo que facilita la auditoría y la supervisión de la seguridad. Los usuarios de la API con el rol de administrador de la organización pueden ver la dirección de correo electrónico del usuario que inició sesión incluyendo el...  `includeUserData=true`` parámetro como en el siguiente:  `/audit/<account_id>?includeUserData=true` .
