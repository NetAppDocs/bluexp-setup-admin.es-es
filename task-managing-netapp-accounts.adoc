...
https://["Vea los extremos de la API de tenancy"^]
...
.Pasos
. Si el usuario aún no lo ha hecho, pida al usuario que vaya a. https://["Sitio web de NetApp BlueXP"^] y regístrese.
...
Para realizar llamadas API al https://["API de tenancy"^], necesitará obtener un token del portador para una cuenta de servicio.

https://["Aprenda a crear un token de cuenta de servicio"^]
