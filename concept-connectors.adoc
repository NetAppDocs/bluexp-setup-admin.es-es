...
Si ejecuta BlueXP  en modo estándar, no necesitará instalar un conector para ejecutar algunos de los servicios de BlueXP , como las actualizaciones de software y la cartera digital. Sin embargo, algunos de los servicios premium de BlueXP , como la protección contra ransomware o la recuperación ante desastres, requieren la instalación de un conector. Si utilizas BlueXP  en modo restringido o privado, tendrás que instalar un conector.



== Qué puede hacer sin un conector en modo estándar

Puede empezar a utilizar BlueXP  para gestionar de forma centralizada sus recursos de almacenamiento sin instalar un conector si se ejecuta en modo estándar.
...
+
La única vez que se requiere un conector para agregar una licencia a la cartera digital es para licencias Cloud Volumes ONTAP _node-based_. Se necesita un conector porque BlueXP  necesita acceder a los datos de licencia instalados en sus sistemas Cloud Volumes ONTAP.
...
Cuando utilice BlueXP  en modo estándar, deberá instalar uno o más conectores para habilitar las siguientes funciones y servicios en BlueXP :
...
Debido a que los conectores son una parte fundamental de la arquitectura de servicio de BlueXP , debe asegurarse de que sus conectores relevantes estén activos, operativos y accesibles en todo momento. Mientras que el servicio está diseñado para superar breves interrupciones de la disponibilidad del conector, debe tomar medidas inmediatas cuando sea necesario para solucionar fallos en la infraestructura.
...
Puede instalar Connectors en un entorno de hiperescalador compatible, o en sus instalaciones, según sus necesidades.

* Amazon Web Services
* Microsoft Azure
+
Implemente su conector en la misma región de Azure que los sistemas Cloud Volumes ONTAP que gestiona, o en https://docs.microsoft.com/en-us/azure/availability-zones/cross-region-replication-azure#azure-cross-region-replication-pairings-for-all-geographies["Par de regiones de Azure"^] el para los sistemas Cloud Volumes ONTAP. Esto garantiza que se utilice una conexión de enlace privado de Azure entre Cloud Volumes ONTAP y sus cuentas de almacenamiento asociadas. https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-enabling-private-link.html["Conozca cómo Cloud Volumes ONTAP utiliza un enlace privado de Azure"^]
...
NetApp normalmente actualiza el software Connector cada mes para introducir nuevas funciones y proporcionar mejoras de estabilidad. Mientras que la mayoría de los servicios y características de la plataforma BlueXP  se ofrecen a través de software basado en SaaS, algunas características dependen de la versión del conector. Que incluye gestión de Cloud Volumes ONTAP, gestión de clústeres ONTAP en las instalaciones, configuración y ayuda.
...
Puede utilizar el mismo conector para gestionar varios entornos de trabajo en BlueXP . El número máximo de entornos de trabajo que debe gestionar un único conector varía. Depende del tipo de entorno laboral, del número de volúmenes, de la cantidad de capacidad que se administra y del número de usuarios.

Si tiene una puesta en marcha a gran escala, trabaje con su representante de NetApp para dimensionar el entorno.

Otra consideración a la hora de determinar cuántos conectores necesita es dónde reside su almacenamiento. Por ejemplo, si tiene Cloud Volumes ONTAP tanto en Google Cloud como en Azure, necesitará un conector para cada entorno. En los casos en los que su almacenamiento exista por completo en las instalaciones, puede ejecutar el conector en cualquier proveedor a hiperescala o en sus instalaciones.
