...
NOTE: Si desea utilizar BlueXP en https://["Entorno de servicios de cloud comercial (C2S) de AWS"^] A continuación, debe seguir instrucciones independientes que describen cómo preparar, instalar el conector e iniciar Cloud Volumes ONTAP. https://["Descubra cómo comenzar a utilizar Cloud Volumes ONTAP en el entorno AWS C2S"^]
...
.Paso
. https://["Habilite las siguientes API de Google Cloud en su proyecto"^]
+
